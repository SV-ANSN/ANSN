...
function SafeLoad(url, name)
    ...
    if not success or not content or content == "" then
        getgenv().Notify({
            Title = 'ANSN | 错误',
            Content = '下载 ' .. (name or "脚本") .. ' 失败，已重试 3 次。',
            Duration = 5
        })
        return function() end
    end
    ...
    if not func then
        warn("ANSN | SafeLoad: 解析 " .. (name or "脚本") .. " 失败: " .. tostring(err))
        return function() end
    end

    local ok, res = pcall(func)
    if not ok then
        warn("ANSN | SafeLoad: 执行 " .. (name or "脚本") .. " 出错: " .. tostring(res))
        return function() end
    end
    return res
end

SafeLoad("https://raw.githubusercontent.com/7yd7/Menu-7yd7/refs/heads/Script/GUIS/Off-site/Notify.lua", "通知系统")
...
local SettingsLib = SafeLoad("https://raw.githubusercontent.com/7yd7/Hub/refs/heads/Branch/GUIS/Settings.lua", "设置库")
...
DiscordBtn.MouseButton1Click:Connect(function()
    ...
    getgenv().Notify({Title = "Discord", Content = "Discord 邀请链接已复制", Duration = 3})
end)
...
local TogglesUI = {}
local GeneralTab = SettingsLib.CreateTab("通用", 1)
TogglesUI.NotifyEnabled = SettingsLib.AddToggle(GeneralTab, "显示通知", "接收提示与反馈", Config.NotifyEnabled, function(v)
    Config.NotifyEnabled = v
    SaveConfig()
end)

TogglesUI.AuthenticFirstPage = SettingsLib.AddToggle(GeneralTab, "原版表情页", "在第一页显示你拥有的表情", Config.AuthenticFirstPage, function(v)
...
local randomModes = { "全部", "收藏" }
local randomDropdown = SettingsLib.AddDropdown(GeneralTab, "随机来源", randomModes, Config.RandomMode or "全部", function(v)
    Config.RandomMode = v
    SaveConfig()
end)
if randomDropdown and randomDropdown.Button then
    randomDropdown.Button.Text = (Config.RandomMode or "全部") .. "  ▼"
end

TogglesUI.RandomEnabled = SettingsLib.AddToggle(GeneralTab, "启用随机", "开启/关闭随机槽位", Config.RandomEnabled, function(v)
...
local CleanFavItem = SettingsLib.AddItem(GeneralTab, "清理已删除的收藏", "扫描所有收藏（表情 + 动画），移除已被删除的项目（点两次）")
...
cleanDeletedFavorites = function()
    ...
    if totalChecks == 0 then
        getgenv().Notify({ Title = "ANSN | 清理", Content = "没有可检查的收藏！", Duration = 3 })
        cleanFavCleaning = false
        resetCleanButton()
        return
    end

    getgenv().Notify({ Title = "ANSN | 清理", Content = "正在检查 " .. totalChecks .. " 个收藏...", Duration = 3 })
    ...
    getgenv().Notify({
        Title = "ANSN | 清理完成",
        Content = "已移除 " .. removedEmotes .. " 个已删除表情 和 " .. removedAnims .. " 个已删除动画",
        Duration = 5
    })
    cleanFavCleaning = false
    resetCleanButton()
...
local ButtonsTab = SettingsLib.CreateTab("按钮", 2)

TogglesUI.SearchVisible = SettingsLib.AddToggle(ButtonsTab, "搜索栏", "显示/隐藏搜索输入框", Config.SearchVisible, function(v)
    Config.SearchVisible = v
    ApplyUIVisibility()
    SaveConfig()
end)

TogglesUI.FavVisible = SettingsLib.AddToggle(ButtonsTab, "收藏按钮", "显示/隐藏星标按钮", Config.FavVisible, function(v)
    Config.FavVisible = v
    ApplyUIVisibility()
    SaveConfig()
end)

TogglesUI.ModeVisible = SettingsLib.AddToggle(ButtonsTab, "模式切换", "显示/隐藏动画模式按钮", Config.ModeVisible, function(v)
    Config.ModeVisible = v
    ApplyUIVisibility()
    SaveConfig()
end)

TogglesUI.FreezeVisible = SettingsLib.AddToggle(ButtonsTab, "冻结按钮", "显示/隐藏表情冻结按钮", Config.FreezeVisible, function(v)
    Config.FreezeVisible = v
    ApplyUIVisibility()
    SaveConfig()
end)

TogglesUI.SpeedVisible = SettingsLib.AddToggle(ButtonsTab, "速度按钮", "显示/隐藏速度控制器", Config.SpeedVisible, function(v)
    Config.SpeedVisible = v
    ApplyUIVisibility()
    SaveConfig()
end)

TogglesUI.NavVisible = SettingsLib.AddToggle(ButtonsTab, "翻页控件", "显示/隐藏翻页按钮", Config.NavVisible, function(v)
    Config.NavVisible = v
    ApplyUIVisibility()
    SaveConfig()
end)

TogglesUI.DiscordVisible = SettingsLib.AddToggle(ButtonsTab, "Discord 按钮", "显示/隐藏 Discord 链接按钮", Config.DiscordVisible, function(v)
...
local hudEditorItem = SettingsLib.AddItem(ButtonsTab, "HUD 编辑器", "重新摆放按钮和界面元素")
...
-- 性能优化：已移除 RenderStepped 循环
-- game:GetService("RunService").RenderStepped:Connect(function()
--     updateGUIColors()
-- end)

local ThemeTab = SettingsLib.CreateTab("主题", 3)

local DiscordPromo = SettingsLib.AddItem(ThemeTab, "想要主题？", "加入我们的 Discord 获取主题！")
...
CopyBtn.MouseButton1Click:Connect(function()
    setclipboard("https://discord.gg/kRfzv2kV7X")
    getgenv().Notify({Title = "Discord", Content = "链接已复制到剪贴板！", Duration = 3})
end)
...
function ApplyTheme(themeData)
    ...
    if not themeData then
        warn("7yd7 | ApplyTheme: themeData 为空，回退到 Default。")
    ...
    if not ok then
        warn("7yd7 | ApplyTheme 错误: " .. tostring(err))
    end
end
...
themeDropdown = SettingsLib.AddDropdown(ThemeTab, "选择主题", GetNames(), currentThemeName, function(v)
    currentThemeName = v
...
local BtnItem = SettingsLib.AddItem(ThemeTab, "主题管理", "管理你的主题")
...
SettingsLib.AddIconButton(BtnRow, "108445456753346", function()
    local popup, content = CreatePopup("创建主题")
    local In = CreateInput(content, "主题名称...")
    
    local Save = CreateButton(content, "保存", (State.EmoteTheme and State.EmoteTheme.Accent) or Color3.fromRGB(0, 255, 150), UDim2.new(0.05, 0, 0.6, 0))
    local Cancel = CreateButton(content, "取消", Color3.fromRGB(50, 50, 50), UDim2.new(0.55, 0, 0.6, 0))
...
SettingsLib.AddIconButton(BtnRow, "117761881427472", function()
    ...
    local popup, content = CreatePopup("重命名主题")
    local In = CreateInput(content, "新名称...", currentThemeName)
    
    local Save = CreateButton(content, "重命名", (State.EmoteTheme and State.EmoteTheme.Accent) or Color3.fromRGB(0, 255, 150), UDim2.new(0.05, 0, 0.6, 0))
    local Cancel = CreateButton(content, "取消", Color3.fromRGB(50, 50, 50), UDim2.new(0.55, 0, 0.6, 0))
    Cancel.TextColor3 = Color3.new(1,1,1)
...
SettingsLib.AddIconButton(BtnRow, "78317476576895", function()
    local popup, content = CreatePopup("导入主题", UDim2.fromOffset(320, 240))
    local box = CreateInput(content, "在此粘贴主题 JSON...", "", true)
    box.Size = UDim2.new(0.9, 0, 0, 130)
    
    local imp = CreateButton(content, "导入主题", (State.EmoteTheme and State.EmoteTheme.Accent) or Color3.fromRGB(0, 255, 150), UDim2.new(0.05, 0, 0.8, 0), UDim2.new(0.9, 0, 0, 35))
    ...
    imp.MouseButton1Click:Connect(function()
        ...
        if s and type(d) == "table" and d.name then
            if d.name == "Default" then
                getgenv().Notify({Title = "错误", Content = "无法覆盖 'Default' 主题。", Duration = 3})
                return
            end
            ...
            getgenv().Notify({Title = "错误", Content = "无效的 JSON 格式！", Duration = 3})
        end
    end)
...
SettingsLib.AddIconButton(BtnRow, "107588515524752", function()
    ...
    local popup, content = CreatePopup("导出主题", UDim2.fromOffset(320, 240))
    local box = CreateInput(content, "", json, true)
    box.Size = UDim2.new(0.9, 0, 0, 130)
    box.TextEditable = false
    
    local copy = CreateButton(content, "复制到剪贴板", (State.EmoteTheme and State.EmoteTheme.Accent) or Color3.fromRGB(0, 255, 150), UDim2.new(0.05, 0, 0.8, 0), UDim2.new(0.9, 0, 0, 35))

    copy.MouseButton1Click:Connect(function()
        setclipboard(json)
        copy.Text = "已复制！"
        task.delay(1, function() copy.Text = "复制到剪贴板" end)
    end)
...
function SmartUpdate(key, subkey, val)
    if currentThemeName == "Default" then
        getgenv().Notify({Title = "主题", Content = "无法修改 Default 主题，请新建一个！", Duration = 2})
        return
    end
...
local WheelFolder = SettingsLib.AddFolder(ThemeTab, "轮盘设置")
...
function AddWheelInput(title, wheelKey)
    ...
    local comp = SettingsLib.AddAssetColor(WheelFolder, title, "资源 ID...", current, currentColor, function(text, color)
        if currentThemeName == "Default" then
            getgenv().Notify({Title = "主题", Content = "无法修改 Default 主题！", Duration = 2})
            return
        end
...
AddWheelInput("轮盘背景", "BackgroundImage")
AddWheelInput("选中渐变", "SelectionGradient")
AddWheelInput("选中线条", "SelectionLine")

local BackgroundFolder = SettingsLib.AddFolder(ThemeTab, "背景设置")
BackgroundFolder.Parent.LayoutOrder = 2
UIElements.Background.Main = SettingsLib.AddColorPicker(BackgroundFolder, "主背景", TableToColor(themes[currentThemeName].Background), function(c)
    SmartUpdate("Background", nil, ColorToTable(c))
end)

local IconSettingsFolder = SettingsLib.AddFolder(ThemeTab, "图标设置")
...
function AddAssetInput(title, iconKey)
    ...
    local comp = SettingsLib.AddInputWithColor(IconSettingsFolder, title, "资源 ID...", defaultText, defaultColor, function(text, color)
        ...
        if not s then
            ...
            getgenv().Notify({Title = "错误", Content = "保存颜色失败！", Duration = 3})
        end
    end)
...
AddAssetInput("左箭头", "Left")
AddAssetInput("右箭头", "Right")
AddAssetInput("行走图标", "Walk")
AddAssetInput("速度图标", "Speed")
AddAssetInput("页面图标", "Page")
AddAssetInput("重载图标", "Reload")
AddAssetInput("收藏（星标）", "Favorite")
AddAssetInput("未收藏", "NotFavorite")
...
State.enterCustomAnimationEditor = function(category, animName)
    ...
    if State.currentCustomAnimationName == "Default" then
        getgenv().Notify({ Title = "7yd7 | 错误", Content = "无法编辑 Default 动画套装，请新建一个！", Duration = 3 })
        return
    end
    ...
    getgenv().Notify({ Title = "ANSN | 动画编辑器", Content = "🖱️ 从轮盘选择一个动画，应用到 " .. animName, Duration = 5 })
end

State.CustomAnimTab = SettingsLib.CreateTab("动画", 4)
State.CustomAnimDropdown = SettingsLib.AddDropdown(State.CustomAnimTab, "选择动画", State.CustomAnimations.Order, State.currentCustomAnimationName, function(v)
...
local CustomAnimBtnItem = SettingsLib.AddItem(State.CustomAnimTab, "动画管理", "管理你的动画")
...
function MakeUniqueSetName(baseSets, desiredName)
    ...
    local i = 2
    local candidate = desiredName .. " (导入)"
    if not baseSets[candidate] then return candidate end
    while true do
        candidate = desiredName .. " (导入 " .. i .. ")"
        if not baseSets[candidate] then return candidate end
        i = i + 1
    end
end
...
SettingsLib.AddIconButton(CustomAnimMgtContainer, "108445456753346", function()
    local popup, content = CreatePopup("创建动画")
    local In = CreateInput(content, "动画名称...")
...
SettingsLib.AddIconButton(CustomAnimMgtContainer, "117761881427472", function()
    ...
    local popup, content = CreatePopup("重命名动画")
    local In = CreateInput(content, "新名称...", State.currentCustomAnimationName)
...
SettingsLib.AddIconButton(CustomAnimMgtContainer, "107588515524752", function()
    ...
    local popup, content = CreatePopup("导出动画", UDim2.fromOffset(320, 240))
    local box = CreateInput(content, "", json, true)
    box.Size = UDim2.new(0.9, 0, 0, 130)
    box.TextEditable = false
...
SettingsLib.AddIconButton(CustomAnimMgtContainer, "78317476576895", function()
    local popup, content = CreatePopup("导入动画", UDim2.fromOffset(320, 240))
    local box = CreateInput(content, "在此粘贴动画 JSON...", "", true)
    box.Size = UDim2.new(0.9, 0, 0, 130)
    
    local imp = CreateButton(content, "导入数据", (State.EmoteTheme and State.EmoteTheme.Accent) or Color3.fromRGB(0, 255, 150), UDim2.new(0.05, 0, 0.8, 0), UDim2.new(0.9, 0, 0, 35))
    imp.MouseButton1Click:Connect(function()
        local s, d = pcall(function() return HttpService:JSONDecode(box.Text) end)
        if s and type(d) == "table" then
            if d.Type and d.Type ~= "CustomAnimationSet" then
                getgenv().Notify({ Title = "错误", Content = "备份类型不匹配！", Duration = 3 })
                return
            end
            if type(d.Data) ~= "table" then
                getgenv().Notify({ Title = "错误", Content = "无效的 JSON", Duration = 3 })
                return
            end
            ...
            if refreshCustomAnimationState then refreshCustomAnimationState(false) end
            popup:Destroy()
            getgenv().Notify({ Title = "ANSN | 动画", Content = "✅ 已导入自定义动画", Duration = 3 })
        else
            getgenv().Notify({ Title = "错误", Content = "无效的 JSON", Duration = 3 })
        end
    end)
...
do
    ...
    State.CustomAnimIconControl = SettingsLib.AddAssetColor(State.CustomAnimTab, "图标", "资源 ID 或链接...", currentImage, currentColor, function(text, color)
        local set = State.CustomAnimations.Sets[State.currentCustomAnimationName]
        if not set then return end
        set.__meta = set.__meta or {}
...
function CreateAnimSetUI(folder, cat, name)
    local item = SettingsLib.AddItem(folder, cat .. " - " .. name, "当前 ID: 0")
...
State.CustomAnimFolders.Idle = SettingsLib.AddFolder(State.CustomAnimTab, "待机动画")
...
State.CustomAnimFolders.Movement = SettingsLib.AddFolder(State.CustomAnimTab, "移动动画")
...
State.RefreshCustomAnimUI = function()
    ...
    for _, elem in pairs(State.CustomAnimUIElems) do
        ...
        if desc then
            ...
            desc.Text = "当前 ID: " .. tostring(val)
        end
    end
end
State.RefreshCustomAnimUI()

State.PageTab = SettingsLib.CreateTab("页面", 5)

function GetEmotePageNames()
    return State.EmotePages.Order
end

SettingsLib.AddItem(State.PageTab, "页面配置", "页面允许你保存不同的收藏套装。切换页面即可快速更换轮盘的收藏内容。")

SettingsLib.AddItem(State.PageTab, "表情配置", "管理你的收藏表情配置")

State.PageDropdown = SettingsLib.AddDropdown(State.PageTab, "选择表情页面", GetEmotePageNames(), State.currentEmotePageName, function(v)
    SwitchEmotePage(v)
    State.SaveEmotePages(State.EmotePages)
end)

local EmotePageMgtItem = SettingsLib.AddItem(State.PageTab, "表情页面管理", " ")
...
SettingsLib.AddIconButton(EmotePageMgtContainer, "108445456753346", function()
    local popup, content = CreatePopup("创建表情页面")
    local In = CreateInput(content, "页面名称...")
    local Save = CreateButton(content, "保存", (State.EmoteTheme and State.EmoteTheme.Accent) or Color3.fromRGB(0, 255, 150), UDim2.new(0.05, 0, 0.6, 0))
    local Cancel = CreateButton(content, "取消", Color3.fromRGB(50, 50, 50), UDim2.new(0.55, 0, 0.6, 0))
    Save.MouseButton1Click:Connect(function()
...
SettingsLib.AddIconButton(EmotePageMgtContainer, "117761881427472", function()
    ...
    local popup, content = CreatePopup("重命名表情页面")
    local In = CreateInput(content, "新名称...", State.currentEmotePageName)
    local Save = CreateButton(content, "重命名", (State.EmoteTheme and State.EmoteTheme.Accent) or Color3.fromRGB(0, 255, 150), UDim2.new(0.05, 0, 0.6, 0))
    local Cancel = CreateButton(content, "取消", Color3.fromRGB(50, 50, 50), UDim2.new(0.55, 0, 0.6, 0))
    Save.MouseButton1Click:Connect(function()
        if In.Text ~= "" and not State.EmotePages.Sets[In.Text] then
...
SettingsLib.AddIconButton(EmotePageMgtContainer, "107588515524752", function() 
    ...
    local json = HttpService:JSONEncode(data)
    local popup, content = CreatePopup("导出表情页面", UDim2.fromOffset(320, 240))
    local box = CreateInput(content, "", json, true)
    box.Size = UDim2.new(0.9, 0, 0, 130)
    box.TextEditable = false
    local copy = CreateButton(content, "复制到剪贴板", (State.EmoteTheme and State.EmoteTheme.Accent) or Color3.fromRGB(0, 255, 150), UDim2.new(0.05, 0, 0.8, 0), UDim2.new(0.9, 0, 0, 35))
...
SettingsLib.AddIconButton(EmotePageMgtContainer, "78317476576895", function() 
    local popup, content = CreatePopup("导入表情页面", UDim2.fromOffset(320, 240))
    local box = CreateInput(content, "在此粘贴页面 JSON...", "", true)
    box.Size = UDim2.new(0.9, 0, 0, 130)
    local imp = CreateButton(content, "导入数据", (State.EmoteTheme and State.EmoteTheme.Accent) or Color3.fromRGB(0, 255, 150), UDim2.new(0.05, 0, 0.8, 0), UDim2.new(0.9, 0, 0, 35))
    imp.MouseButton1Click:Connect(function()
        local s, d = pcall(function() return HttpService:JSONDecode(box.Text) end)
        if s and type(d) == "table" and d.Type == "EmotePageSet" and type(d.Data) == "table" then
            ...
            popup:Destroy()
            getgenv().Notify({ Title = "ANSN | 页面", Content = "✅ 已导入表情页面", Duration = 3 })
        else
            getgenv().Notify({ Title = "错误", Content = "无效的表情页面 JSON", Duration = 3 })
        end
    end)
    local close = Instance.new("TextButton")
    close.Size = UDim2.fromOffset(24, 24)
...
SettingsLib.AddItem(State.PageTab, "动画配置", "管理你的收藏动画配置")


local BackupTab = SettingsLib.CreateTab("备份", 6)

local BackupDesc = SettingsLib.AddItem(BackupTab, "备份包含什么？", " ")
...
MakeDescLine("<b>• 主题：</b>保存自定义主题")
MakeDescLine("<b>• 设置：</b>保存 HUD 布局和数值")
MakeDescLine("<b>• 收藏：</b>保存收藏的表情/动画")
MakeDescLine("<b>• 全部：</b>包含以上全部内容")

local ExportItem = SettingsLib.AddItem(BackupTab, "导出设置", "将当前设置保存到文件，方便分享或之后导入。")
...
local BtnExportAll = CreateExportBtn("导出全部设置", btnColors.dark, 1)
local BtnExportThemes = CreateExportBtn("导出主题", btnColors.blue, 2)
local BtnExportSettings = CreateExportBtn("导出设置", btnColors.blue, 3)
local BtnExportFavorites = CreateExportBtn("导出收藏", btnColors.blue, 4)
...
BtnExportAll.MouseButton1Click:Connect(function()
    ...
    BtnExportAll.Text = "已复制！"
    task.delay(1, function() BtnExportAll.Text = "导出全部设置" end)
end)
...
BtnExportThemes.MouseButton1Click:Connect(function()
    ...
    setclipboard(HttpService:JSONEncode(data))
    BtnExportThemes.Text = "已复制！"
    task.delay(1, function() BtnExportThemes.Text = "导出主题" end)
end)
...
BtnExportSettings.MouseButton1Click:Connect(function()
    ...
    setclipboard(HttpService:JSONEncode(data))
    BtnExportSettings.Text = "已复制！"
    task.delay(1, function() BtnExportSettings.Text = "导出设置" end)
end)
...
BtnExportFavorites.MouseButton1Click:Connect(function()
    ...
    setclipboard(HttpService:JSONEncode(data))
    BtnExportFavorites.Text = "已复制！"
    task.delay(1, function() BtnExportFavorites.Text = "导出收藏" end)
end)


local ImportItem = SettingsLib.AddItem(BackupTab, "导入设置", "选择一个备份文件来恢复你的配置，并覆盖当前设置。")
...
local BtnImportAll = CreateImportBtn("导入全部设置", btnColors.dark, 1)
local BtnImportThemes = CreateImportBtn("导入主题", btnColors.blue, 2)
local BtnImportSettings = CreateImportBtn("导入设置", btnColors.blue, 3)
local BtnImportFavorites = CreateImportBtn("导入收藏", btnColors.blue, 4)

function HandleImportPrompt(typeStr)
    local typeLabels = { All = "全部", Themes = "主题", Settings = "设置", Favorites = "收藏" }
    local popup, content = CreatePopup("导入" .. (typeLabels[typeStr] or typeStr), UDim2.fromOffset(320, 240))
    local box = CreateInput(content, "在此粘贴备份 JSON...", "", true)
    box.Size = UDim2.new(0.9, 0, 0, 130)
    ...
    imp.MouseButton1Click:Connect(function()
        ...
        if s and type(d) == "table" and d.Type then
            ...
            getgenv().Notify({Title = "成功", Content = "数据导入成功！", Duration = 3})
            popup:Destroy()
        else
            getgenv().Notify({Title = "错误", Content = "无效的备份 JSON 格式！", Duration = 3})
        end
    end)
...
RunService.Heartbeat:Connect(function()
    ...
    if errorMsg and errorMsg.Visible then
        if player.Character and player.Character:FindFirstChild("Humanoid") and player.Character.Humanoid.RigType == Enum.HumanoidRigType.R6 then
            errorMsg.ErrorText.Text = "只有 R15 可用，R6 不支持"
...
function createGUIElements()
    ...
    UI.Search.PlaceholderText = "搜索/ID"
    UI.Search.Text = ""
...
toggleFavoriteAnimation = function(animationData)
    ...
    if found then
        ...
        getgenv().Notify({
            Title = 'ANSN | 收藏系统',
            Content = '🗑️ 已从收藏中移除 "' .. animationData.name .. '"',
            Duration = 3
        })
    else
        ...
        getgenv().Notify({
            Title = 'ANSN | 收藏系统',
            Content = '✅ 已将 "' .. animationData.name .. '" 加入收藏',
            Duration = 3
        })
    end
...
applyAnimation = function(animationData)
    ...
    if not animate or not humanoid then
        getgenv().Notify({
            Title = 'ANSN | 动画错误',
            Content = '❌ 找不到 Animate 或 Humanoid',
            Duration = 3
        })
        return
    end
    ...
    if index == 1 and randomActive then
        ...
        if State.customAnimationEditorActive then
            ...
            if State.CustomAnimations.Sets[State.currentCustomAnimationName] and cat and name then
                ...
                State.SaveCustomAnimations(State.CustomAnimations)
                getgenv().Notify({ Title = "ANSN | 已保存", Content = "✅ 已保存 " .. name, Duration = 3 })
                if State.RefreshCustomAnimUI then State.RefreshCustomAnimUI() end
                if refreshCustomAnimationState then refreshCustomAnimationState(true) end
                State.exitCustomAnimationEditor()
            end
    ...
    task.spawn(function()
        while true do
            ...
            if emoteData then
                ...
                getgenv().Notify({Title = 'ANSN | 表情', Content = "📦 表情已加载", Duration = 3})
                return
            end
            task.wait(3)
        end
...
enterHUDEditor = function()
    ...
    table.insert(HUD.Connections, lockBtn.MouseButton1Click:Connect(function()
        ...
        getgenv().Notify({ 
            Title = "ANSN | HUD 编辑器", 
            Content = HUD.IsUnlocked and "🔓 内部已解锁！子元素现在可以编辑了。" or "🔒 内部已锁定！仅可编辑顶层元素。", 
            Duration = 2 
        })
    end))
    ...
    table.insert(HUD.Connections, exportBtn.MouseButton1Click:Connect(function()
        ...
        setclipboard(HttpService:JSONEncode(data))
        getgenv().Notify({ Title = "ANSN | HUD 编辑器", Content = "✅ HUD 设置已复制", Duration = 2 })
    end))

    table.insert(HUD.Connections, importBtn.MouseButton1Click:Connect(function()
        local popup, content = CreatePopup("导入 HUD", UDim2.fromOffset(320, 240))
        ...
        local box = CreateInput(scroll, "在此粘贴 HUD JSON...", "", true)
        ...
        local imp = CreateButton(content, "导入 HUD", (State.EmoteTheme and State.EmoteTheme.Accent) or Color3.fromRGB(0, 255, 150), UDim2.new(0.05, 0, 0.8, 0), UDim2.new(0.9, 0, 0, 35))
        ...
        imp.MouseButton1Click:Connect(function()
            ...
            if s and type(d) == "table" then
                ...
                if d.Type and d.Type ~= "HUD" then
                    getgenv().Notify({ Title = "错误", Content = "HUD 导入类型不匹配！", Duration = 3 })
                    return
                end
                if type(settings) ~= "table" then
                    getgenv().Notify({ Title = "错误", Content = "无效的 HUD JSON", Duration = 3 })
                    return
                end
                ...
                popup:Destroy()
                getgenv().Notify({ Title = "ANSN | HUD 编辑器", Content = "✅ HUD 设置已导入", Duration = 2 })
            else
                getgenv().Notify({ Title = "错误", Content = "无效的 HUD JSON", Duration = 3 })
            end
        end)
    ...
    table.insert(HUD.Connections, resetBtn.MouseButton1Click:Connect(function()
        ...
        getgenv().Notify({ Title = "ANSN | HUD 编辑器", Content = "🔄 所有设计和框架已完全重置", Duration = 3 })
    end))
    ...
    local _, posBox = createPropRow("位置", 1)
    local _, sizeBox = createPropRow("尺寸", 2)
    local zRow, zBox = createPropRow("层级", 3)
    local bgRow, bgBox = createPropRow("背景透明", 4)
    local bgcRow, bgcBox = createPropRow("背景颜色", 5)
    local imgRow, imgBox = createPropRow("图片透明", 6)
    local imgcRow, imgcBox = createPropRow("图片颜色", 7)
    local radRow, radBox = createPropRow("圆角", 8)
    local txtRow, txtBox = createPropRow("文本", 9, true)
    local phRow, phBox = createPropRow("占位符", 10, true)
    local ttrRow, ttrBox = createPropRow("文本透明", 11)
    local txtcRow, txtcBox = createPropRow("文本颜色", 12)
    ...
    table.insert(HUD.Connections, RunService.RenderStepped:Connect(function()
        ...
        if e and e.Parent then
            title.Text = string.format("[%s] %s", e.ClassName, eName or "未知")
    ...
    table.insert(HUD.Connections, deleteBtn.MouseButton1Click:Connect(function()
        ...
        ApplyUIVisibility()
        pcall(function() updateGUIColors() end)
        getgenv().Notify({ Title = "ANSN | HUD 编辑器", Content = "🗑️ 自定义框架已删除", Duration = 2 })
    end))
    ...
    table.insert(HUD.Connections, addBtn.MouseButton1Click:Connect(function()
        ...
        getgenv().Notify({ Title = "ANSN | HUD 编辑器", Content = "➕ 已添加自定义框架！", Duration = 2 })
    end))

    getgenv().Notify({ Title = "ANSN | HUD 编辑器", Content = "✏️ 拖动元素即可重新摆放", Duration = 5 })
end
...
if UserInputService.TouchEnabled and not UserInputService.KeyboardEnabled then
    SafeLoad("https://raw.githubusercontent.com/7yd7/Hub/refs/heads/Branch/GUIS/OpenEmote.lua", "打开表情菜单")
    getgenv().Notify({
        Title = 'ANSN | 手机端表情',
        Content = '📱 已添加打开表情菜单的按钮，方便使用',
        Duration = 10
    })
end
